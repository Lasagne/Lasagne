...
class Layer(object):
    ...
    def get_params(self):
        ...
        Notes
        -----
        By default this returns an empty list, but it should be overridden in a
        subclass that has trainable parameters.
        """
        return []

    def get_bias_params(self):
        """
        Returns a list of all the Theano variables that are bias parameters for
        the layer.
        ...
        parameters. This is useful when specifying regularization (it is often
        undesirable to regularize bias parameters).
        """
        return []
    ...
    def get_output_shape(self):
        ...
        -------
        output shape: tuple
            A tuple that represents the output shape of this layer. The tuple
            has as many elements as there are output dimensions, and the
            elements of the tuple are either integers or `None`.

        Notes
        -----
        When implementing a new :class:`Layer` class, you will usually keep
        this unchanged and just override `get_output_shape_for()`.
        """
    ...
    def get_output(self, input=None, **kwargs):
        ...
        input : None, Theano expression, numpy array, or dict
            If None, uses the inputs of the :class:`InputLayer` instances. If a
            Theano expression, this will replace the inputs of all
            :class:`InputLayer` instances (useful if your network has a single
                input layer).
            If a numpy array, this will be wrapped as a Theano constant and
            used just like a Theano expression.
            If a dictionary, any :class:`Layer` instance (including the input
            layers) can be mapped to a Theano expression or numpy array to use
            instead of its regular output.
        ...
        -----
        When implementing a new :class:`Layer` class, you will usually keep
        this unchanged and just override `get_output_for()`.
        """
    ...
    def get_output_for(self, input, **kwargs):
        ...
        Notes
        -----
        This is called by the base :class:`Layer` implementation to propagate
        data through a network in `get_output()`. While `get_output()` asks the
        underlying layers for input and thus returns an expression for a
        layer's output in terms of the network's input, `get_output_for()` just
        performs a single step and returns an expression for a layer's output
        in terms of that layer's input.
    ...
    def create_param(self, param, shape, name=None):
        ...
        shape : tuple
            A tuple of integers representing the desired shape of the parameter
            array.

        Returns
        -------
        Theano shared variable
            A Theano shared variable representing layer parameters. If a numpy
            array was provided, the variable is initialized to contain this
            array. If a shared variable was provided, it is simply returned. If
            a callable was provided, it is called, and its output is used to
            initialize the variable.

        Notes
        -----
        This method should be used in `__init__()` when creating a
        :class:`Layer` subclass that has trainable parameters. This enables the
        layer to support initialization with numpy arrays, existing Theano
        shared variables, and callables for generating initial parameter
        values.
        """
        if name is not None:
...
class MultipleInputsLayer(Layer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        Notes
        -----
        This is called by the base :class:`MultipleInputsLayer` implementation
        to propagate data through a network in `get_output()`. While
        `get_output()` asks the underlying layers for input and thus returns an
        expression for a layer's output in terms of the network's input,
        `get_output_for()` just performs a single step and returns an
        expression for a layer's output in terms of that layer's input.
