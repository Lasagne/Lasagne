...
class TestSpatialDropoutLayer:
    ...
    @pytest.fixture
    def layer(self, input_layer):
        from lasagne.layers.noise import SpatialDropoutLayer
        return SpatialDropoutLayer(input_layer)

    @pytest.fixture
    def layer_no_rescale(self, input_layer):
        from lasagne.layers.noise import SpatialDropoutLayer
        return SpatialDropoutLayer(input_layer, rescale=False)

    @pytest.fixture
    def layer_p_02(self, input_layer):
        from lasagne.layers.noise import SpatialDropoutLayer
        return SpatialDropoutLayer(input_layer, p=0.2)

    def test_get_output_for_non_deterministic(self, layer):
        input = theano.shared(numpy.ones((10, 100, 100, 100)))
        result = layer.get_output_for(input)
        result_eval = result.eval()
        assert 0.9 < result_eval.mean() < 1.1
        assert (numpy.unique(result_eval) == [0., 2.]).all()
    ...
    def test_get_output_for_p_float32(self, input_layer):
        from lasagne.layers.noise import SpatialDropoutLayer
        layer = SpatialDropoutLayer(input_layer, p=numpy.float32(0.5))
        input = theano.shared(numpy.ones((10, 100, 100, 100),
                                         dtype=numpy.float32))
        assert layer.get_output_for(input).dtype == input.dtype
    ...
    def test_specified_rng(self, input_layer):
        ...
        set_rng(RandomState(seed))
        result = SpatialDropoutLayer(input_layer).get_output_for(input)
        result_eval1 = result.eval()
    ...
    def test_feature_level_dropout(self, layer_no_rescale):
        input = theano.shared(numpy.ones((10, 100, 100, 100)))
        result = layer_no_rescale.get_output_for(input)
        result_eval = result.eval()

        for image in result_eval:
            for features in image:
                assert features.sum() in [0, 10000]
